...
export class EchoMindApp extends LitElement {
    static styles = css`
        :host {
            display: block;
            width: 100%;
    ...
    connectedCallback() {
        ...
        if (window.api) {
            window.api.echoMindApp.onClickThroughToggled((_, isEnabled) => {
    ...
    disconnectedCallback() {
        ...
        if (window.api) {
            window.api.echoMindApp.removeAllClickThroughListeners();
        }
    }
...
customElements.define('echo-mind-app', EchoMindApp);
