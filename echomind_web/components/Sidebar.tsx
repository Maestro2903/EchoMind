            ...
            <header className={`group relative h-6 flex shrink-0 items-center justify-between`}>
                {isCollapsed ? (
                    <button
                        ...
                        className={`${isCollapsed ? '' : ''} text-gray-500 hover:text-gray-800 p-1 rounded-[4px] hover:bg-[#f7f7f7] h-6 w-6 transition-colors focus:outline-none`}
                        aria-label="Open sidebar"
                    >
                        <Image src="/unfold.svg" alt="Open sidebar" width={18} height={18} className="h-4.5 w-4.5" />
                    </button>
                ) : (
                    <>
                        <Link href="https://echomind.com" target="_blank" rel="noopener noreferrer" className="flex items-center">
                            <span className="ml-3 text-[14px] leading-none text-[#000]">EchoMind</span>
                        </Link>
                        <button
                            onClick={toggleSidebar}
                            onKeyDown={e => handleKeyDown(e, toggleSidebar)}
                ...
                {!isCollapsed && hasApiKey !== null && (
                    <div className="px-2.5 py-2 text-center">
                        <span className={`text-xs px-2 py-1 rounded-full ${hasApiKey ? 'bg-blue-100 text-blue-800' : 'bg-green-100 text-green-800'}`}>
                            {hasApiKey ? 'Local running' : 'EchoMind Free System'}
                        </span>
                    </div>
                )}

                {/* Additional links removed per requirement */}
